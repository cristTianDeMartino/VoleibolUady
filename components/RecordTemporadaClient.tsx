...
const mkScores = (n: number) => Array.from({ length: n }, () => ({ uady: '', rival: '' }))
...
export default function RecordTemporadaClient({ partidos, isAdmin }: Props) {
  ...
  function updateScore(idx: number, side: 'uady' | 'rival', value: string) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        {isAdmin && (
          <button
            ...
            className="bg-uady-orange-cta text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:brightness-110 transition-all duration-200 shrink-0"
          >
            + Registrar Partido
          </button>
        )}
          ...
          {filtrados.map((p) => {
            ...
            return (
              ...
              >
                ...
                {/* Cuerpo: UADY vs rival + sede */}
                <div className="p-4 flex-1">
                  <p className="text-center text-lg font-black text-uady-blue">
                    UADY <span className="text-uady-gold">vs</span> {p.rival}
                  </p>
      ...
      {isAdmin && showForm && (
        ...
        >
          ...
          >
            {/* Header del modal */}
            <div className="bg-uady-orange-cta px-5 py-3 rounded-t-2xl flex items-center justify-between">
              ...
              <form ref={formRef} action={formAction} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Sede" required span2>
                  <input
                    name="sede"
                    required
                    placeholder="Ej. Gimnasio Polifuncional UADY"
                    className={inputCls}
                  />
                </Field>
                ...
                <div className="sm:col-span-2">
                  <p className="text-xs font-bold text-gray-600 mb-2">
                    Parciales por Set <span className="text-uady-orange-cta">*</span>
                  </p>
                      ...
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-1">
                          <div className="flex-1">
                            <label className="text-[10px] text-gray-400 font-medium block mb-0.5">UADY</label>
                            <input
                              type="number"
                              min={0}
                              max={40}
                              value={score.uady}
                              onChange={(e) => updateScore(i, 'uady', e.target.value)}
                              placeholder="0"
